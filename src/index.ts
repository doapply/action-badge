...
const validatebadgeIdAsUUID = (badge_id: string): boolean => {
	...
	return validate(badge_id.replace(uuid4Regex, '$1-$2-$3-$4-$5'))
}
	...
	; (async function () {
		try {
			...
			const badge_id = core.getInput('badge_id')
			...
			if (!validatebadgeIdAsUUID(badge_id)) {
				throw new Error(`Invalid badge_id: ${badge_id}`)
			}

			console.log(`Dryrun`, dryrun)

			// Fetch the latest badge
			try {
				const res = await fetch(badgeURL(badge_id))
				badgeContent = await res.text()
				const images = await fetchImagesFromSVG(badgeContent)

				for (const image in images) {
					if (Object.prototype.hasOwnProperty.call(images, image)) {
						badgeContent = badgeContent.replace(image, await convertImageToBase64(images[image]))
					}
				}

				await fs.mkdir(path.dirname(path.join(`/tmp`, filename)), { recursive: true })
				await fs.writeFile(path.join(`/tmp`, filename), badgeContent)
				...
				if (filename.endsWith('.png')) {
					...
					console.log('Converted badge to PNG', 'ok')
				}
			...
			// Retrieve previous SHA of badge file to be able to update content through the API
			try {
				const {
					repository: {
						object: { oid },
			...
			// Compare previous to current badge
			const git = sgit()
			const sha = await git.hashObject(path.join(`/tmp`, filename))
			console.log('Current badge sha', sha)
